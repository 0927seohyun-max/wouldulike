...
function drawEndingComment() {
  ...
  drawCommentBox(width / 2, y1, "김소이", "팀원과 긴밀히 협력하는 과정 속에서 직접 게임 구조를 만들어보니 흥미를 느꼈고 교수님이 중간평가 때 주신 피드백을 통해 어떻게 하면 게임에 재미를 느낄 수 있는 요소를 넣을지에 대해 고민하는 과정에서 협의와 수정을 반복하면서 많은 배움을 얻을 수 있었다. 이번 기회를 발판 삼아 JavaScript라는 언어로 한층 더 고도화된 미디어 아트를 만들어보고 싶다는 확신이 들었고 결과적으로 팀과 함께한 프로젝트가 잘 마무리 되어서 뿌듯하다.");
  drawCommentBox(width / 2, y2, "남서현", "처음에는 단순히 고정된 코드를 작성해나가는 과정이라고만 생각했는데, 직접 게임의 모든 흐름을 통제하고 완성해 보니 무형의 아이디어를 시각화해내는 고유한 재미를 깊게 깨달았다. 예기치 못한 치명적인 오류들을 실시간으로 알아내고 기능을 유기적으로 조립해 가는 과정 속에서 엄청난 성취감을 축적할 수 있었고, 전반적인 시각 디자인과 레이아웃 구성까지 복합적으로 고찰하며 몰입도를 끌어올렸다. 소통과 협업의 값진 가치를 깨달을 수 있는 소중한 경험이었던 것 같다.");
  drawCommentBox(width / 2, y3, "최명빈", "캔버스에 우주 이미지를 동적으로 출력하고 물체의 이동과 입자 이펙트 효과를 직관적으로 구현해내는 작업 전반이 매우 재미있었다. 단순한 액션 게임을 넘어 명확한 목표 지점과 스토리가 공존하는 게임을 설계했다는 점이 재미있었다. 추후 입체 사운드 밸런스를 강화해 빌드를 발전시키고 싶다. 팀원들과 함께하는 프로젝트여서 힘든 부분도 많았지만 결과적으로 잘 끝나서 뿌듯했다.");
}
